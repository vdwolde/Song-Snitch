...
| `host:createRoom` | host | `{ songsPerPlayer }` | Replaces the room wholesale |
...
| `player:unsubmit` | player | `{ trackId }` | Lobby only, your own submissions only |
...
| `t` | Sent to | Notes |
| --- | --- | --- |
| `error` | The one socket that triggered it | `{ code, message }` — `code` is one of `bad-room \| name-taken \| colour-taken \| room-full \| game-in-progress \| not-host \| not-premium \| spotify` |
